...
export async function POST(request: NextRequest) {
  ...
  const name = String(body.name || 'Quick workout').trim() || 'Quick workout';
  const muscleGroup = String(body.muscle_group || 'Full body').trim() || 'Full body';
  const reps = Number.isFinite(body.reps) ? Number(body.reps) : Number(body.reps || 10);
  ...
  const setInsert = await supabase
    .from('workout_sets')
    .insert({ user_id: user.id, workout_id: workoutInsert.data.id, muscle_group: muscleGroup, reps, completed: true, created_at: now });
